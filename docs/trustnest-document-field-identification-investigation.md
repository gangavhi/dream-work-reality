...
2. **Spatial label→value pairing mis-associates** OCR blocks (single-word values like `PATEL` treated as labels), producing wrong canonical keys and junk extension fields.
...
| Field extraction | Context-aware mapping to `ProfileSchema` keys | **Unreliable** — wrong keys/values on passport E2E; simulated passport blocks produce `legal_last_name=Name`, `legal_middle_name=Date Of Birth Patel Given` |
...
Any single token ≤24 letters (e.g. `PATEL`, `PRIYA`) is classified as a **label**, so horizontal/vertical pairing attaches the wrong neighbor as its “value”. That flows into `OpenVocabularyFieldExtractor` → wrong `ProfileSchema` keys.
...
legal_middle_name=Date Of Birth Patel Given
...
|----------|-----|-----------|
| **P0** | Tighten `isFieldLabel` — require known label phrases or trailing `:`; **never** treat bare proper-noun tokens as labels | Stops PATEL/PRIYA mis-pairing (Breakpoint D) |
