...
|-------|--------------------------------|
| **Layout pairing treats single tokens as “labels”** (`OcrLayoutSerializer`) | Values like `PATEL` mis-paired → wrong keys for **last name**, **first name** |
...
`legal_last_name=Name`, `legal_middle_name=Date Of Birth Patel Given` — classic **pairing + heuristic** failure, not ONNX.
