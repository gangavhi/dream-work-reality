...
| **Manual-only profile (no document upload or scan)** | Users who **do not** want to upload, photograph, or import documents must still be able to **enter information once** in the app (guided household/person forms), have it **persist in local memory**, and use it for **browser and in-app form filling**—same capabilities as data that arrived via OCR, minus document-linked artifacts. This path is **first-class**, not a fallback: it matches **privacy preference** (no scans on disk), **speed** (faster than finding paper), **control** (only what they choose to type), and **access** (they may know facts from memory or another device without a file here). Product copy should present **“Enter manually”** alongside **scan / upload** with equal prominence where appropriate. |
| **Basic provenance** | At minimum: source type (**manual** vs OCR vs **generative ingest**), and link to source document when applicable (**manual** entries have no document link unless the user later attaches one). |
...
| **Roles and relationships** | Each person can carry **labels** (e.g. Father, Mother, Child 1, Child 2) and **relationship edges** (parent/guardian of, partner of) so the product knows **who is linked to whom**—required for **multi-profile** forms (child as subject, parent fields from the correct adults). |
| **Profile picker for forms** | UI to choose **which household member** the form is **primarily about** (form subject); **not** limited to single-profile data—see next row. |
| **Multi-profile form fill** | For one form session, map fields to the **right person** by **semantic role** (patient/child vs parent/guardian vs emergency contact) using roles + heuristics + per-site overrides—e.g. pediatric medical: **child** demographics + **parent** phone/email without treating the household as one flat profile. **Undifferentiated** parent lines may pull **either** parent’s stored value; **Father’s** / **Mother’s**-specific inputs bind to **that** profile only. Session UI shows **form about [X]** and **also using [Y, Z]** when multiple members contribute. |
| **Generative resolution of incomplete household rows** | **Only** for **undifferentiated** fields (e.g. **parent/guardian** phone with **no** Father/Mother specificity): if one parent’s row lacks a number but the **other** parent has one, **on-device model** may **select** it with **provenance**. For **role-specific** fields (**Father’s** phone, **Mother’s** email, **Parent 1** …): value **must** come from **that** person’s row; if **missing**, **prompt the user**—**no** substitution from the other parent. **Sensitive** field classes **excluded** from cross-person inference by policy. User can **override** generic picks. |
...
| **Household profile (subject) for this form** | User **selects or confirms which person** the form is **primarily for**—**self, spouse/partner, each child, or other member**. **Fills, gaps, saves, and attachments** resolve **per field** to the correct **Person** using **roles and relationships**; **generative fallback** across parents **only** where the **form semantics** are **undifferentiated** (any parent contact OK). **Role-specific** missing fields → **user prompt**, not **borrow** from the other parent. Optional **per-site or per-link** remembered defaults (user-controlled). |
| **Session indicator** | Persistent **“Form about: [Name]”** (or equivalent); when multiple members contribute, compact **“Also using: [roles/names]”** with **Change** per scope before applying sensitive data. |
| **Inject or apply stored values** | Map local fields to inputs via heuristics + saved per-site overrides + **role-aware** resolution + **generative household resolution** **only** for **undifferentiated** parent/guardian-type fields when one parent’s row is empty; **role-specific** inputs bind to **one** profile **only**. |
| **Detect gaps vs local memory** | After applying rules above, **surface a clear prompt** for **true** gaps—especially **role-specific** values not in local memory (e.g. **Father’s** phone missing)—and for **generic** lines only when **no** suitable value exists **anywhere** in the linked household. |
...
| Feature | Why it matters |
|--------|----------------|
| **Curated “form packs” by vertical** | Pre-tested **semantic mappings** for healthcare (patient vs guarantor), school (student vs guardian), sports leagues—includes **generic** vs **Father/Mother/Parent-N** field detection—reduces wrong-profile swaps on complex multi-column forms. |
...
| Phase | Focus |
|-------|--------|
| **MVP** | §1 mandatory: local store + **generative ingest** (default on-device) + dynamic schema + **manual-only entry path** (add/edit people and fields **without** scan or file—persisted for form fill; provenance **manual**) + household UI + **roles/relationships** + **form subject** + **multi-profile fill** (at least child + parent/guardian split) + **generative resolution** for **undifferentiated** parent/guardian fields when one parent row is incomplete (**cross-parent** fallback + provenance); **role-specific** father/mother/parent-N fields → **prompt** if missing (**sensitive** fields policy-defined) + **optional retained document files** (opt-in, key types) + **Word/Excel import** (at least **xlsx/docx** parse + generative map + **edit**; **no** macro execution) + **folder batch import** (where OS supports folder/multi pick; **automated persist** + **import activity**) + **Chrome extension + companion app** (form detection, user-invoked fill, native-autofill coexistence, **attach from saved** where possible) + optional **in-app** assist on mobile/tablet where feasible + account boundary + baseline security + **core** proximity sharing (same-ecosystem first if needed). **Watched-folder** incremental import is **not** required for MVP cut (see **Hardening**). |
...
- **Mandatory** = local structured memory with **generative structuring** (default on-device), **dynamic schema** in SQLite, **Word/Excel import** with on-device parsing and **generative map + edit**, **user-chosen folder batch import** with on-device OCR and **fully automated persist** plus **import activity** for optional fixes, **watched-folder incremental import** (same pipeline when **new files** appear—**platform-dependent** watch vs rescan; see §1.2 row and §3 **Hardening** phasing), **household roles and relationships** (e.g. father/mother/child labels, parent-of links) with **form-subject** selection, **multi-profile form fill**, and **generative resolution** for **undifferentiated** parent lines plus **prompts** for missing **role-specific** fields—with session indicator and **change** affordance, **optional retained document files** (user-controlled, on-device, for uploads), **Chrome extension + companion app** form assistance (form-only activation, coexistence with browser autofill, **attach from saved documents** where applicable), **gap detection** with prompts to enter missing information and **optional save** of **new** values (**insert**), plus **optional update** of **existing** local values when the user changes information on the form and confirms (**insert + update**, never silent), tablet/phone **in-app** paths where feasible, review/fast paths and OTP-friendly UX, strict backend minimization, household CRUD UI, proximity-based selective sharing with expiry and revoke, and a security baseline—and **manual-only data entry** (no scan/upload required) so users who prefer **not** to bring documents into the app still get **one-time entry → persistent local memory → form assist** with the same fill and governance features as OCR-backed data (provenance: **user-entered**).
