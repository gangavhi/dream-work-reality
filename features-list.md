...
| **Multi-profile form fill** | For one form session, map fields to the **right person** by **semantic role** (patient/child vs parent/guardian vs emergency contact) using roles + heuristics + per-site overrides—e.g. pediatric medical: **child** demographics + **parent** phone/email without treating the household as one flat profile. Session UI shows **form about [X]** and **also using [Y, Z]** when multiple members contribute. |
| **Generative resolution of incomplete household rows** | When the **preferred** profile (e.g. **Mother**) lacks a value but **another** linked household member (e.g. **Father**) has a **usable** attribute for that **semantic need** (e.g. only mobile on file), **on-device generative model** **selects** source + **confidence** + **provenance** (“filled from Casey—Jordan’s mobile not on file”) instead of **always** leaving blanks or **forcing** manual entry. **Sensitive** field classes **excluded** from cross-person inference by policy. User can **override**; low-confidence cases still **prompt**. |
...
| **Household profile (subject) for this form** | User **selects or confirms which person** the form is **primarily for**—**self, spouse/partner, each child, or other member**. **Fills, gaps, saves, and attachments** resolve **per field** to the correct **Person** (subject vs parent/guardian, etc.) using **roles and relationships** and **generative fallback** across members when **canonical** rows are incomplete; **ask explicitly** when the model is **ambiguous** (e.g. two competing values for the same role-specific line); optional **per-site or per-link** remembered defaults (user-controlled). |
...
| **Inject or apply stored values** | Map local fields to inputs via heuristics + saved per-site overrides + **role-aware** resolution + **generative household resolution** (which profile supplies **patient** vs **parent** block; **which alternate member** when the primary row is empty)—not only a single **active** profile. |
| **Detect gaps vs local memory** | Before or while filling, identify **required or requested** fields still **unresolved** after **generative household resolution** (including cross-member fallback where policy allows); **surface a clear prompt** only for **true** gaps—not for fields the model could fill from **existing** household data. |
...
| Feature | Why it matters |
|--------|----------------|
| **Curated “form packs” by vertical** | Pre-tested **semantic mappings** for healthcare (patient vs guarantor), school (student vs guardian), sports leagues—reduces wrong-profile swaps on complex multi-column forms. |
