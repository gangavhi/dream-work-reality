...
| **Household profile (subject) for this form** | User **selects or confirms which person** the form is **primarily for**—**self, spouse/partner, each child, or other member**. **Fills, gaps, saves, and attachments** resolve **per field** to the correct **Person** (subject vs parent/guardian, etc.) using **roles and relationships**; **ask explicitly** when ambiguous (e.g. which parent when two exist); optional **per-site or per-link** remembered defaults (user-controlled). |
...
| **Inject or apply stored values** | Map local fields to inputs via heuristics + saved per-site overrides + **role-aware** resolution (which profile supplies **patient** vs **parent** block)—not only a single **active** profile. |
