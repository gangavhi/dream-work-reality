...
- **Manual entry without documents (first-class):** Users who **do not** want to upload, scan, or import files must still be able to **type information once** into structured household/person screens, have it **saved in the same local relational store**, and use it for **form assistance**—identical downstream behavior to OCR-derived rows for fill, gaps, history, and sharing (subject to what exists in memory). **Importance:** supports **strict privacy preference** (no document images on device), **speed** when the user already knows the facts, **access** when physical documents are unavailable, and **parity** so the product is never “OCR-only.” **Usefulness:** reduces friction for onboarding and ongoing updates without touching the capture pipeline; provenance records **manual** vs document-backed sources.
- **Several complementary ways to bring documents in** (capture and parsing **on-device**; **default** generative structuring **on-device**; none sends raw document content to your backend)—**optional** for users who prefer manual-only memory:
...
**Field mapping** connects local schema fields to page inputs (heuristics + saved per-site overrides + user nudges + **generative household resolution** where policy allows) in **whichever** surface is active (extension DOM, in-app WebView, or in-app native form mapping). For **multi-person** forms, mapping is **role-aware**: inputs are matched not only to **field names** but to **semantic roles** (**patient/child**, **parent/guardian**, **subscriber**, **emergency contact**) and resolved to the correct **Person** via **household relationships**—so a pediatric form can pull **child demographics** and **parent contact** in one pass without treating them as one blob. **Critical distinction:** for **undifferentiated** parent/guardian lines (any parent’s number will do), the generative layer may pick **either** parent’s stored contact when one is missing; for **role-specific** lines (**Father’s** phone, **Mother’s** email), values **must** come from **that** person’s row—if **empty**, **prompt the user**—**no** substitution from the other parent.
...
- **Incomplete canonical data (real households):** **Not every adult** has a full row for every field. The product must distinguish **two cases**:
  - **Undifferentiated parent/guardian contact:** The form asks for a **parent or guardian** phone (or similar) **without** requiring **Father** vs **Mother** specifically. **Either** parent’s stored number may be used when only one exists—**generative resolution** picks the **best available** with **provenance** (e.g. **“Using Jordan’s number—only parent mobile on file”** when the label is generic).
  - **Role-specific fields:** The form asks for **Father’s** contact number, **Mother’s** email, **Parent 1** phone, etc. The value **must** come from **that** person’s profile. If **Father’s** number is **not** in local memory, the product **does not** fill it from **Mother’s** row—**prompt the user** to enter it (or capture it). **No** cross-parent substitution for **explicitly role-bound** inputs; that is a **hard rule**, not a confidence tradeoff.
- **Generative household field resolution (fill-time)** applies to the **first** case and to other **semantic slack** situations (e.g. **emergency contact** when only one adult has a mobile). For the **second** case, resolution is **disabled**: missing **role-specific** data → **user assist**, not **borrow**.
- **Prefer asking** when the **form subject** is ambiguous: **“Who is this form for?”** with **Me**, **My partner**, and **each child** (by name), plus **Other household member**. For **role-specific** gaps, the prompt is **targeted** (e.g. **“We don’t have Father’s phone on file—add it now?”**), not a generic empty box.
...
- **Gap detection** and **save / update prompts** must be **field-aware**: “Missing **Sam’s** date of birth” vs “Missing **Jordan’s** phone for guardian block”—and write-back targets the **correct** person’s row, not only the form subject. **Role-specific** missing fields **never** auto-fill from the wrong person’s row.
