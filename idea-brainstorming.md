...
- **Batch folder import**: **enumerate → OCR/parse → generative structure → persist → review list for fixes**; optional **skip duplicates** (file hash), **pause/resume**, **max files per run** to protect battery and storage; **assign person** per item or default + bulk-edit—wrong assignment is a top risk, so UX must make **who this belongs to** explicit (before or immediately after ingest).
...
- **Data inventory** screen: what exists, how big, last changed, export/delete; **batch imports** listed as **sessions** (date, folder label, count, **review/correction status**) so users can audit what came from a **folder sweep**; **watched folders** listed with **last activity**, **paused** state, and **files processed** count.
