...
- **Several complementary ways to bring documents in** (all processed **on-device**; none sends raw document content to your backend):
...
- **Batch folder import**: **enumerate → OCR → queue → confirm**; optional **skip duplicates** (file hash), **pause/resume**, **max files per run** to protect battery and storage; **assign person** per item or default + bulk-edit—wrong assignment is a top risk, so UX must make **who this belongs to** explicit before commit.
- **Word / Excel**: **parse → preview table or extracted fields → map columns/labels → confirm**; treat **merged cells**, **multiple sheets**, and **embedded images** as edge cases with **manual** fallback; **values** preferred over formulas where both appear (product-defined display of formula result if parsed).

### 4.3 Trust and transparency

- **Data inventory** screen: what exists, how big, last changed, export/delete; **batch imports** listed as **sessions** (date, folder label, count, confirm status) so users can audit what came from a **folder sweep**.
...
10. **Folder import limits**: max files per batch, max total size, supported extensions, and **iOS/Android** folder-picker behavior vs **desktop** full-directory access.
