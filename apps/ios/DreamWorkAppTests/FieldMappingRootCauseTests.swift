...
final class FieldMappingRootCauseTests: XCTestCase {
    ...
    func testPassportSpatialBlocksMislabelSingleWordValuesAsLabels() async {
        ...
        XCTAssertTrue(layout.labelValuePairs.contains { $0.label == "Surname" && $0.value == "PATEL" })
        XCTAssertTrue(result.suggestions.contains { $0.profileKey == ProfileFieldKey.legalLastName && $0.value == "PATEL" })
        ...
        let badLabels = layout.labelValuePairs.filter { pair in
            ["PATEL", "PRIYA", "INDIAN"].contains(pair.label.uppercased())
        }
