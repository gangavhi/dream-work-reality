...
final class OcrLayoutSerializerTests: XCTestCase {
    ...
    func testVerticalLabelValuePairsForPassportLayout() {
        let blocks = [
            ...
            OcrLayoutSerializer.LayoutBlock(text: "PATEL", confidence: 1, x: 0.1, y: 0.68, width: 0.25, height: 0.03),
            OcrLayoutSerializer.LayoutBlock(text: "Given Name(s)", confidence: 1, x: 0.1, y: 0.62, width: 0.25, height: 0.03),
            OcrLayoutSerializer.LayoutBlock(text: "AMIT", confidence: 1, x: 0.1, y: 0.58, width: 0.2, height: 0.03),
        ]
        let pairs = OcrLayoutSerializer.labelValuePairs(from: blocks)
        let byLabel = Dictionary(uniqueKeysWithValues: pairs.map { ($0.label.uppercased(), $0.value) })
        XCTAssertEqual(byLabel["SURNAME"], "PATEL")
        XCTAssertEqual(byLabel["GIVEN NAME(S)"], "AMIT")
    }
