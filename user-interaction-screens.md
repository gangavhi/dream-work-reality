...
- **Other fields** on the same form (guardian, subscriber, billing) are filled from the **correct** other profiles using those roles—**seamlessly**, with a short line on screen such as **“Form about: Sam · Also using: Jordan (parent), Casey (parent)”** so nothing feels hidden. **Real households rarely have every field for every adult:** if **Jordan** has no mobile on file but **Casey** does, the helper **does not** leave “mother’s cell” blank by default—the **generative assistant** **decides** whether a **parent/guardian** line can be filled from the **best available** household number (with a short **why**, e.g. **“Only mobile on file—Casey”**) instead of making the user hunt. If two parents exist and the form **requires** a specific parent’s number and **both** have different numbers, the app may **ask once** when the model **cannot** disambiguate safely.  
