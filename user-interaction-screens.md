...
- **Other fields** on the same form (guardian, subscriber, billing) are filled from the **correct** other profiles using those roles—**seamlessly**, with a short line on screen such as **“Form about: Sam · Also using: Jordan (parent), Casey (parent)”** so nothing feels hidden. **Two rules:** (1) If the form asks for a **parent or guardian** contact in a **generic** way (no **Father** vs **Mother** in that line), **either** parent’s number on file can be used when only one exists—the helper shows **why** (e.g. **“Using Casey’s number—only parent mobile saved”**). (2) If the form asks for **Father’s** phone (or **Mother’s**, **Parent 1**, etc.) and that value **is not** saved for that person, the app **does not** drop in the other parent’s number—it **asks** the user to add or type it. If two parents exist and a **generic** line needs one number and **both** have different mobiles, the app may **ask once** which to use.  
