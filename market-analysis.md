...
| **No desire to scan or upload documents** (privacy, simplicity, or no files on hand) | **One-time manual entry** into the app: users **type** what they want remembered; data **persists locally** and powers the **same** form-filling and household features as OCR-backed memory—**without** document images or imports. Reduces barrier to adoption and matches users who trust **typed facts** but not **retained scans**. |
...
| **Large piles of existing scans** (e.g. one folder per school year) | **User-chosen folder import**: enumerate supported files, **batch OCR on-device**, **fully automated** generative ingest into local memory (**no** confirmation queue before commit); **import activity** for optional correction—still **no** cloud document processing by default. Optional **watched folder**: **new files** trigger the **same** pipeline automatically (where the OS allows **watch** or **rescan**). |
...
| **Multi-person households** | User sets **roles** (e.g. father, mother, children) and **relationships**; picks a **form subject** (who the form is about). Forms that need **more than one member** on the same page (e.g. child **patient** + **parent/guardian** contact) resolve fields to the **correct** profile **seamlessly**—not a single flat “family blob.” When **canonical** rows are **incomplete**, **on-device generative resolution** applies to **undifferentiated** parent/guardian contact (either parent’s number may serve); **role-specific** fields (**Father’s** / **Mother’s** …) **prompt** if missing—**no** wrong-parent substitution—reducing **errors** and **busywork** without uploading data to a server. |
...
12. **Per-person form scope + multi-profile fields**  
   Explicit **form subject** (who the form is **about**) plus **roles** (parent, child, guardian) so **one page** can pull **child** and **parent** facts from the **right** profiles. **Generic** parent/guardian lines can use **either** parent’s stored contact when needed; **Father’s** / **Mother’s**-labeled fields stay **role-true**—**prompt** if missing, **no** silent swap from the other parent.
